...
const TABS = [
  { id: 'all', label: 'Tümü' },
  { id: 'robot_vacuum', label: 'Robot Süpürge' },
  { id: 'stick_vacuum', label: 'Dikey Süpürge' },
  { id: 'airfryer', label: 'Airfryer & Pişirici' },
  { id: 'coffee', label: 'Kahve Makineleri' },
  { id: 'climate', label: 'Hava Temizleyici & Klima' }
];
      ...
      .filter((p) => {
        ...
        if (activeTab === 'robot_vacuum') {
          ...
          return (
            ...
            pName.includes('magnifica') ||
            pName.includes('lattego') ||
            pName.includes('delonghi')
          );
        } else if (activeTab === 'climate') {
          return (
            subCat.includes('air_purifier') ||
            subCat.includes('air_conditioner') ||
            pName.includes('klima') ||
            pName.includes('hava temiz') ||
            pName.includes('purifier') ||
            pName.includes('inverter')
          );
        }
    ...
    <div className="space-y-8 pb-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="pt-4 pb-2 text-center space-y-3">
        <div className="text-xs text-slate-400 font-semibold flex items-center justify-center gap-1.5">
          ...
          <span>/</span>
          <span className="text-slate-800 font-bold">Ev ve Yaşam Teknolojileri</span>
        </div>
        ...
        <p className="text-xs sm:text-sm text-slate-500 font-medium max-w-md mx-auto">
          Robot süpürgeler, dikey süpürgeler, kahve makineleri, hava temizleyiciler ve canlı mağaza fiyat karşılaştırmaları
        </p>
        ...
        <div className="max-w-xl mx-auto pt-2 flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Ev aleti ara (model, marka, emiş gücü, özellik)..."
              value={searchQuery}
              onChange={(e) => {
      ...
      {displayProducts.length > 0 ? (
        ...
        <div className="bg-slate-50 border border-slate-200/80 rounded-3xl p-12 text-center space-y-3">
          <p className="text-sm font-bold text-slate-700">Seçilen filtrelere uygun ev teknolojisi ürünü bulunamadı.</p>
          <button
            onClick={() => {
              setSearchQuery('');
