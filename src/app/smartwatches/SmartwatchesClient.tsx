...
const TABS = [
  { id: 'all', label: 'Tüm Akıllı Saatler', desc: 'Tüm tasarım ve modeller' },
  { id: 'premium', label: 'Premium & Titanyum', desc: 'Apple Watch Ultra, Galaxy Watch Ultra & Safir cam' },
  { id: 'sports', label: 'Spor & Outdoor (GPS)', desc: 'Garmin, Suunto, Amazfit dayanıklı spor saatleri' },
  { id: 'daily', label: 'Günlük & Klasik', desc: 'Şık çelik/deri kayış & hafif tasarım' },
  { id: 'band', label: 'Akıllı Bileklik', desc: 'Uzun pil ömrü & hafif sağlık takibi' }
];
      ...
      .filter((p) => {
        ...
        if (activeTab === 'premium') {
          ...
          return (
            ...
            pName.includes('series 10') ||
            pName.includes('series 9') ||
    ...
    <div className="space-y-6 pb-16 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
      ...
      <div className="pt-2 space-y-3">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              type="text"
              placeholder="Akıllı saat ara (Apple Watch, Garmin, marka)..."
              value={searchQuery}
              onChange={(e) => {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pt-1">
            ...
            >
              ...
              {brandDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-72 sm:w-80 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-3 grid grid-cols-2 gap-1.5 z-50 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  >
                    Tüm Saat Markaları ({products.length})
                  </button>
          ...
          <Link
            href="/smartwatches?sortBy=popular"
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <span className="text-[11px] font-black tracking-wide text-emerald-300 block uppercase">
                  Akıllı Saat Fırsatları
                </span>
                <span className="text-xs font-bold text-slate-200 group-hover:text-white transition-colors">
                  MediaMarkt & Amazon Canlı Giyilebilir İndirimleri
                </span>
              </div>
            </div>
      ...
      {displayProducts.length > 0 ? (
        <div className="space-y-10 pt-2">
          ...
          {visibleCount < displayProducts.length && (
            <div className="text-center pt-4">
              ...
              >
                <span>Daha Fazla Saat Göster ({displayProducts.length - visibleCount} model kaldı)</span>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-12 text-center space-y-3 shadow-xs">
          <p className="text-sm font-bold text-slate-700 dark:text-slate-300">Seçilen filtrelere uygun saat modeli bulunamadı.</p>
          <button
            onClick={() => {
              setSearchQuery('');
              handleSelectBrand('all');
...
export function SmartwatchesClient({ initialProducts }: { initialProducts: Product[] }) {
  return (
...
export default SmartwatchesClient;
