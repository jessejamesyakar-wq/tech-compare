...
function preFilterProducts(userMessage: string, limit = 45): CatalogItem[] {
  ...
  // Extract potential budget numbers (e.g., "15000", "15.000", "50k", etc.)
  ...
  const scored = allProducts.map((p) => {
    ...
    // Keywords in name or highlights
...
// Fallback algorithm if Anthropic API key is not configured or in development
function generateFallbackResponse(userMessage: string, candidates: CatalogItem[]): AssistantResponse {
  ...
  if (topItems.length === 0) {
    return {
      reply: "Aradığınız kriterlere uygun ürünleri inceliyorum. Lütfen bütçenizi veya istediğiniz özellikleri belirterek tekrar yazınız.",
      recommendations: [],
    };
  }
  ...
  const recommendations: AssistantRecommendation[] = topItems.map((item) => ({
    ...
    price: item.price,
    reason: `${item.brand} ekosisteminde ₺${item.price.toLocaleString("tr-TR")} güncel piyasa fiyatı ve güçlü donanımıyla öne çıkıyor.`,
  }));

  return {
    reply: `Sizin için kataloğumuzdaki 600+ ürün arasından en uygun ${recommendations.length} modeli listeledim. Detayları ve mağaza fiyatlarını kartlara tıklayarak inceleyebilirsiniz.`,
    recommendations,
  };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // If API key is missing or dummy placeholder, use the intelligent catalog matching engine
    if (!apiKey || apiKey === "your_anthropic_api_key_here" || apiKey.trim() === "") {
      console.warn("[AI Assistant] ANTHROPIC_API_KEY tanımlı değil veya placeholder. Algoritmik katalog önerisi kullanılıyor.");
...
Kurallar:
- SADECE aşağıda verilen katalogdaki ürünleri öner, katalog dışı veya uydurma ürün ekleme.
- Fiyat, kategori, marka ve özellik uyumuna dikkat et.
- Kullanıcının bütçesini aştıysan veya tam bütçesine denk getirdiysen bunu açıkça belirt.
- Yanıtını SADECE aşağıdaki geçerli JSON formatında ver, başına veya sonuna başka hiçbir metin ekleme:

{
  "reply": "Kullanıcıya samimi, tarafsız ve açıklayıcı bir Türkçe yanıt (2-3 cümle)",
  "recommendations": [
    {
      "productId": "ürünün id'si",
      "slug": "ürünün slug'ı",
      "productName": "tam ürün adı",
      "category": "ürünün category'si (örn. phones, tvs, laptops, appliances, headphones, tablets, smartwatches, monitors, consoles)",
      "price": 45000,
      "reason": "bu ürünün kullanıcının isteğine neden uygun olduğuna dair net 1 cümle"
    }
  ]
}

Eğer katalogda kullanıcının isteğine uygun ürün yoksa, recommendations dizisini boş bırak ve reply alanında durumu nazikçe açıkla.
...
${JSON.stringify(relevantProducts, null, 2)}`;
    ...
    if (!claudeResponse.ok) {
      ...
      console.error("[AI Assistant] Claude API hatası:", errText);
      const fallback = generateFallbackResponse(body.message, relevantProducts);
      return NextResponse.json(fallback);
    }
    ...
    // Claude bazen ```json ... ``` bloğu içinde dönebilir, temizleyelim.
    const cleaned = rawText.replace(/```json|```/g, "").trim();

    let parsed: AssistantResponse;
    try {
      parsed = JSON.parse(cleaned);
      // Ensure recommendation fields are well-formed
    ...
    console.error("[AI Assistant] Endpoint hatası:", err);
    return NextResponse.json(
