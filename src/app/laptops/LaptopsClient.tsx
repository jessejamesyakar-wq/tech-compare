...
const TABS = [
  { id: 'all', label: 'Tüm Laptop & PC', desc: 'Tüm taşınabilir ve masaüstü sistemler' },
  { id: 'gaming', label: 'Oyun Bilgisayarı (RTX)', desc: 'Yüksek FPS, RTX 40/50 & hızlı ekran' },
  { id: 'ultrabook', label: 'Ultrabook & MacBook', desc: 'Hafif, uzun pil ömrü & şık kasa' },
  { id: 'workstation', label: 'İş & Mühendislik', desc: '32GB+ RAM, CAD & render gücü' },
  { id: 'daily', label: 'Günlük & Öğrenci', desc: 'Ekonomik, hafif ofis ve okul için' }
];
    ...
    <div className="space-y-6 pb-16 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
            ...
            >
              ...
              {brandDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-72 sm:w-80 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-3 grid grid-cols-2 gap-1.5 z-50 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  >
                    Tüm Bilgisayar Markaları ({products.length})
                  </button>
      ...
      {displayProducts.length > 0 ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-12 text-center space-y-3 shadow-xs">
          <p className="text-sm font-bold text-slate-700 dark:text-slate-300">Seçilen filtrelere uygun bilgisayar modeli bulunamadı.</p>
          <button
            onClick={() => {
              setSearchQuery('');
              handleSelectBrand('all');
...
export function LaptopsClient({ initialLaptops }: { initialLaptops: LaptopProduct[] }) {
  return (
