...
const TABS = [
  { id: 'all', label: 'Tüm Tabletler', desc: 'Tüm boyut ve performans seviyeleri' },
  { id: 'flagship', label: 'Amiral Gemisi & Pro (M4/S10)', desc: 'En güçlü işlemci ve OLED ekran' },
  { id: 'air_mid', label: 'Air & Orta Segment', desc: 'İdeal fiyat/performans & kalem desteği' },
  { id: 'compact', label: 'Kompakt & Mini (8-9")', desc: 'Tek elle taşınabilir küçük ekran' },
  { id: 'entry', label: 'Öğrenci & Giriş Seviyesi', desc: 'Bütçe dostu eğitim ve video için' }
];
  ...
  const displayProducts = useMemo(() => {
    return products
      .filter((p) => {
        ...
        const screenSize = parseFloat(specs.screenSize || specs.screen?.size || '0');
        ...
        if (activeTab === 'flagship') {
          return (
            ...
            pName.includes('m4') ||
            pName.includes('s10') ||
            pName.includes('s9 ultra') ||
            pName.includes('s9+') ||
            pName.includes('matepad pro')
          );
        } else if (activeTab === 'air_mid') {
          return (
            pName.includes('air') ||
            pName.includes('fe') ||
            pName.includes('pad 6') ||
            pName.includes('pad 7') ||
            pName.includes('redmi pad') ||
            pName.includes('magicpad') ||
            pName.includes('oneplus pad')
          );
        } else if (activeTab === 'compact') {
          return (
            pName.includes('mini') ||
            (screenSize > 0 && screenSize <= 9.0) ||
            pName.includes('8.4') ||
            pName.includes('8.7')
          );
          ...
          return (
            p.basePrice < 10000 ||
            pName.includes('tab a') ||
            pName.includes('t30') ||
            pName.includes('t20') ||
            pName.includes('matepad se')
          );
        }
    ...
    <div className="space-y-6 pb-16 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
      ...
      <div className="pt-2 space-y-3">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              type="text"
              placeholder="Tablet ara (iPad, Galaxy Tab, marka)..."
              value={searchQuery}
              onChange={(e) => {
            ...
            >
              ...
              {brandDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-72 sm:w-80 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-3 grid grid-cols-2 gap-1.5 z-50 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  >
                    Tüm Tablet Markaları ({products.length})
                  </button>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <span className="text-xs font-bold text-slate-200 group-hover:text-white transition-colors">
                  Hepsiburada & Trendyol Canlı Tablet İndirimleri
                </span>
              </div>
            </div>
      ...
      {displayProducts.length > 0 ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-12 text-center space-y-3 shadow-xs">
          <p className="text-sm font-bold text-slate-700 dark:text-slate-300">Seçilen filtrelere uygun tablet modeli bulunamadı.</p>
          <button
            onClick={() => {
              setSearchQuery('');
              handleSelectBrand('all');
...
export function TabletsClient({ initialProducts }: { initialProducts: Product[] }) {
  return (
