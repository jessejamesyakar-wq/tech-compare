...
import { filterSmartphones } from '@/lib/data';
...
interface PhoneCatalogContentProps {
  initialPhones: Smartphone[];
  initialBrands: string[];
  initialBrandCounts: Record<string, number>;
}

function PhoneCatalogContent({ initialPhones, initialBrands, initialBrandCounts }: PhoneCatalogContentProps) {
  const { t } = useI18n();
  ...
  const [brandCounts] = useState<Record<string, number>>(initialBrandCounts);
...
export default function PhonesClient(props: PhoneCatalogContentProps) {
  return (
    <Suspense fallback={<div className="py-24 text-center text-slate-500 font-bold">Yükleniyor...</div>}>
      <PhoneCatalogContent {...props} />
    </Suspense>
  );
}
