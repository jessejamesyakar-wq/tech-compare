...
import { getConsoleSpecRows } from '@/lib/productPresentation';
...
export default function ConsolesDetailClient({ initialProduct }: { initialProduct: Product | null }) {
  ...
  if (!product) {
    return (
      <div className="py-24 text-center space-y-4">
        ...
        <Link href="/consoles" className="text-emerald-600 font-bold text-xs underline">
          Oyun Konsolları Kataloğuna Dön
        </Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50/50 pb-20 pt-4">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 overflow-x-auto py-1">
          ...
          <Link href="/consoles" className="hover:text-emerald-600 transition-colors">Oyun Konsolları</Link>
          <ChevronRight className="w-3.5 h-3.5 text-slate-400 shrink-0" />
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {getConsoleSpecRows(specs).map(({key, label, value}) => {
              return (
                <div key={key} className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  <span className="text-xs font-black text-slate-900 break-words min-w-0">{value}</span>
                </div>
              );
            })}
          </div>
