...
(async () => {
  console.log('=== HARVESTING OFFICIAL EPEY CANONICAL PHOTOS (HTML REGEX MATCH) ===\n');
  ...
  for (const item of epeyDirectPages) {
    try {
      console.log(`Loading Epey page for: ${item.name}...`);
      await page.goto(item.url, { waitUntil: 'domcontentloaded', timeout: 25000 });
      // Short delay for client hydrate
      await new Promise(r => setTimeout(r, 1200));

      const html = await page.content();
      // Match all resim.epey.com product image URLs
      const matches = html.match(/https:\/\/resim\.epey\.com\/[0-9]+\/(?:b_|m_|s_|k_|z_)[^\"]+\.(?:png|jpg|webp)/gi);

      if (matches && matches.length > 0) {
        // Pick first product image
        const best = matches.find(m => !m.includes('logo') && !m.includes('site/') && !m.includes('grup/')) || matches[0];
        const bigImg = best.replace('/m_', '/b_').replace('/s_', '/b_').replace('/k_', '/b_').replace('/z_', '/b_');
        ...
        if (ok) {
          ...
          console.log(`  ✅ SAVED OFFICIAL EPEY PHOTO: ${localPath} (${bigImg})`);
          results.push({
            id: item.id,
            name: item.name,
            epeyImage: localPath,
        ...
        console.log(`  ⚠️ No match in HTML for ${item.name}`);
      }
