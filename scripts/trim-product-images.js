...
async function collectCatalogImages() {
  ...
  for (const d of datasets) {
    ...
    for (const p of products) {
      ...
      if (Array.isArray(p.images)) {
        p.images.forEach((i) => images.add(i));
      }
      if (Array.isArray(p.colors)) {
        p.colors.forEach((c) => {
          if (c.image) images.add(c.image);
          if (Array.isArray(c.images)) c.images.forEach((i) => images.add(i));
        });
      }
    }
  }
...
async function trimAndRePadFile(fullPath, paddingPercent = 0.06, threshold = 10) {
  try {
