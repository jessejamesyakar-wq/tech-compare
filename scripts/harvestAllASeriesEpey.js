...
(async () => {
  console.log('=== HARVESTING ALL OFFICIAL GALAXY A-SERIES PHOTOS FROM EPEY ===\n');
  ...
  for (const item of directEpeyUrls) {
    try {
      ...
      if (matches && matches.length > 0) {
        // Find product image
        const productMatch = matches.find(m => !m.includes('logo') && !m.includes('site/') && !m.includes('grup/')) || matches[0];
        const bigImg = productMatch.replace('/m_', '/b_').replace('/s_', '/b_').replace('/k_', '/b_').replace('/z_', '/b_');
  ...
  fs.writeFileSync(path.join(dataDir, 'epey_a_series_final_harvest.json'), JSON.stringify(results, null, 2), 'utf8');

  console.log(`\n=== GALAXY A-SERIES HARVEST COMPLETE: ${results.length} / ${directEpeyUrls.length} DOWNLOADED! ===`);
})();
